...
export default function InvoicePage() {
  ...
  function selectCatalogItem(rowId: number, item: InventoryItem) {
    setRows(rows.map(r =>
      r.id === rowId
        ? { ...r, itemTag: item.code, itemName: item.name, description: item.description, qty: 1, price: item.salePrice, account: item.account, taxRate: item.taxRate, amount: item.salePrice }
        : r
    ))
    setOpenDropdownRow(null)
    setHoveredCatalog(null)
  ...
  function getAvailableQty(itemName: string): number | null {
    const inv = catalog.find(i => i.name === itemName)
    return inv ? inv.quantity : null
  }

  function rowHasQtyIssue(row: LineItem): boolean {
    if (!row.itemName || row.qty === '') return false
    const available = getAvailableQty(row.itemName)
    return available !== null && Number(row.qty) > available
  }

  const anyQtyIssue = rows.some(rowHasQtyIssue)

  async function deductInventory() {
    const soldRows = rows.filter(r => r.itemName && r.qty !== '' && Number(r.qty) > 0)
    await Promise.all(soldRows.map(async row => {
    ...
    const invoiceItems = rows
      ...
      .map(r => ({
        ...
        backordered: rowHasQtyIssue(r),
      }))
    await fetch('/api/invoices', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ to: 'Customer', date: '11 Apr 2022', dueDate: '11 Apr 2022', due: total, items: invoiceItems }),
    })
  }
  ...
  function handleApproveClick() {
    if (anyQtyIssue) {
      ...
      deductInventory().then(() => setApproved(true))
    }
  }
  ...
  // ── Frame 8: Approved read-only view ────────────────────────────────────────
  if (approved) {
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid #e3e5e8', padding: '8px 24px 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <h1 style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>New Invoice</h1>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={{ display: 'flex' }}>
                <button style={{ background: '#0078C8', color: '#fff', border: 'none', borderRadius: '4px 0 0 4px', padding: '5px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>
                  Add Payment
                </button>
        ...
        <div style={{ padding: '24px', maxWidth: 1000, width: '100%', margin: '0 auto' }}>
          <div style={{ background: '#fff', borderRadius: 4, border: '1px solid #e3e5e8', padding: '20px 24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 20, marginBottom: 24 }}>
              {[
                { label: 'To', value: 'Andy Spark', blue: true },
                { label: 'Issue date', value: '11 Apr 2022' },
                { label: 'Due date', value: '11 Apr 2022' },
                { label: 'Invoice number', value: 'INV-1234' },
                { label: 'Online payments', value: 'democp@democp.co', sub: 'Manage' },
              ].map(({ label, value, blue, sub }) => (
                <div key={label}>
                  <div style={{ fontSize: 11, color: '#666', marginBottom: 4 }}>{label}</div>
                  <div style={{ fontSize: 13, color: blue ? '#0078C8' : '#1a1a1a', fontWeight: blue ? 500 : 400 }}>{value}</div>
                  {sub && <div style={{ fontSize: 12, color: '#0078C8', cursor: 'pointer', marginTop: 2 }}>{sub}</div>}
                </div>
              ))}
            </div>
              ...
              <thead>
                <tr style={{ borderBottom: '2px solid #e3e5e8' }}>
                  {['Item/description', 'Qty', 'Price', 'Tax amount', 'Amount'].map((h, i) => (
                    <th key={h} style={{ padding: '8px 8px', textAlign: i === 0 ? 'left' : 'right', fontSize: 12, fontWeight: 500, color: '#555' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#f4f5f6' }}>
      ...
      <div style={{ padding: '24px', maxWidth: 1200, width: '100%', margin: '0 auto' }}>
        <div style={{ background: '#fff', borderRadius: 4, border: '1px solid #e3e5e8', padding: '20px 20px 0' }}>
          ...
          {/* Form row 1 */}
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.2fr 1.5fr 1.5fr', gap: 16, marginBottom: 4 }}>
            ...
            <FormField label="Document number">
              <div style={{ border: '1px solid #ccc', borderRadius: 4, height: 32, display: 'flex', alignItems: 'center', paddingLeft: 8, gap: 6 }}>
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="#666" strokeWidth="1.5"><rect x="1" y="1" width="10" height="10" rx="1"/><path d="M3 4h6M3 6h6M3 8h4"/></svg>
                <span style={{ fontSize: 13, color: '#333' }}>INV-1234</span>
              </div>
            </FormField>
          ...
          <div style={{ marginBottom: 16 }}>
            <a href="#" style={{ color: '#0078C8', fontSize: 12, textDecoration: 'none' }}>Add sales tax exemption</a>
          </div>

          {/* Form row 2 */}
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.2fr 1.5fr 1.5fr', gap: 16, marginBottom: 20 }}>
            <FormField label="Online payments">
              <div style={{ border: '1px solid #ccc', borderRadius: 4, height: 32, display: 'flex', alignItems: 'center', paddingLeft: 8, color: '#0078C8', fontSize: 13, cursor: 'pointer' }}>
                Manage online payments
              </div>
            </FormField>
            <FormField label="Currency"><SelectField value="New Zealand D.." /></FormField>
            <FormField label="Branding theme"><SelectField value="Standard" /></FormField>
          </div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
              ...
              <tbody>
                {rows.map((row) => {
                  const available = row.itemName ? getAvailableQty(row.itemName) : null
                  const isOver = rowHasQtyIssue(row)
                  return (
                    <tr key={row.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                      {/* Drag handle */}
                      ...
                      <td style={tdStyle}>
                        <div
                          ...
                          style={{
                            border: openDropdownRow === row.id ? '2px solid #0078C8' : '1px solid #ccc',
                            borderRadius: 4, minHeight: 30, padding: '4px 6px',
                            cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 2, background: '#fff',
                          }}
                        >
                          {row.itemTag && (
                      ...
                      {/* Description */}
                      <td style={tdStyle}>
                        <input style={cellInputStyle} value={row.description}
                          onChange={e => updateRow(row.id, 'description', e.target.value)} />
                      </td>

                      {/* Qty — with "X available" hint when over */}
                      <td style={tdStyle}>
                        <input
                          style={{ ...cellInputStyle, textAlign: 'right', border: isOver ? '1px solid #e07b00' : '1px solid transparent', borderRadius: 3 }}
                          value={row.qty} type="number"
                          onChange={e => updateRow(row.id, 'qty', e.target.value === '' ? '' : Number(e.target.value))}
                        />
                        {available !== null && (
                          <div style={{ fontSize: 10, color: isOver ? '#e07b00' : '#888', textAlign: 'right', paddingRight: 4, marginTop: 1 }}>
                            {available} available
                          </div>
                        )}
                      </td>
                      ...
                      {/* Account */}
                      <td style={tdStyle}>
                        <input style={cellInputStyle} value={row.account}
                          onChange={e => updateRow(row.id, 'account', e.target.value)} />
                      </td>

                      {/* Tax rate */}
                      <td style={tdStyle}>
                        <input style={cellInputStyle} value={row.taxRate}
          ...
          {/* Frame 6: qty warning banner */}
          {anyQtyIssue && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', background: '#fffbf0', borderTop: '1px solid #f5e0a0', fontSize: 12, color: '#5a3e00' }}>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0 }}>
                <path d="M7 1L13 12H1L7 1Z" fill="#e07b00" stroke="#e07b00" strokeWidth="0.5" strokeLinejoin="round"/>
                <path d="M7 5v3M7 9.5v.5" stroke="#fff" strokeWidth="1.3" strokeLinecap="round"/>
              </svg>
              <span>
                Some items don't have enough quantity available. Items will be automatically allocated to this invoice when they become available.{' '}
                <a href="#" style={{ color: '#0078C8', textDecoration: 'none' }}>Learn about backorders ↗</a>
              </span>
            </div>
          )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', marginTop: 4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#555' }}>
            ...
            Send as an e-invoice
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#aaa" strokeWidth="1.5"><circle cx="7" cy="7" r="5"/><path d="M7 5v3M7 10v.5"/></svg>
          </div>
        </div>
      </div>
          ...
          {catalog.map(item => (
            <div key={item.id} onClick={() => selectCatalogItem(openDropdownRow, item)}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 2 }}>
                ...
                <span style={{ fontSize: 11, color: '#888' }}>Qty: {item.quantity}  Price: {item.salePrice.toFixed(2)}</span>
              </div>
      ...
      {showApproveModal && createPortal(
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 6, padding: '28px 32px', width: 460, boxShadow: '0 8px 32px rgba(0,0,0,0.2)', position: 'relative' }}>
            ...
            <h2 style={{ margin: '0 0 12px', fontSize: 16, fontWeight: 600 }}>Some items don't have enough quantity available</h2>
            <p style={{ margin: '0 0 24px', fontSize: 13, color: '#555', lineHeight: 1.5 }}>
              Items will be automatically allocated to this invoice when they become available.{' '}
              <a href="#" style={{ color: '#0078C8', textDecoration: 'none' }}>Learn more about backorders ↗</a>
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              ...
              <button onClick={() => { setShowApproveModal(false); deductInventory().then(() => setApproved(true)) }}
